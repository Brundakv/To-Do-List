...
let acceptData = () => {
  data.push({
    ...
    completed: false, // v2 Add completed status
  });
  ...
  createTasks();
  //   resetForm(); same as writing inside createTasks()
};
...
let updateTask = () => {
  ...
  data[editIndex] = {
    ...
    date: dateInput.value,
    description: textarea.value,
    completed: data[editIndex].completed, // v2 Preserve completed status
  };
...
modal.addEventListener("hidden.bs.modal", () => {
  ...
  add.innerHTML = "Add"; // Ensure button text resets to "Add"
});
...
let editTask = (e) => {
  ...
  // Update these selectors to get content from the name-date div
  ...
  textarea.value = selectedTask.children[1].innerHTML; // Description is now the second child
...
//v2 Add new function to toggle completion status
//as an argument in functions like toggleComplete(${y}) to identify which task is being modified.
